...
Use `sql_schemas` to expose a cube in one or more schemas in the SQL API. The
...
explicitly when both qualified and unqualified access are required. Cubes that
omit this parameter remain available in `public` for backward compatibility.
An extending cube inherits `sql_schemas` when it omits the parameter; declaring
...
or start with `pg_`. A cube with [`public: false`](#parameters-public) is not
exposed in any SQL schema.
